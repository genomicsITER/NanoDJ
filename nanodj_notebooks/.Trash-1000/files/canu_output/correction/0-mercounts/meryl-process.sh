...
export CANU_OBJECT_STORE_NAMESPACE=agalactiae
...
if [ -e ./agalactiae.ms16.histogram -a -e ./agalactiae.ms16.dump -a -e ./agalactiae.ms16.ignore.gz ] ; then
  exit 0
fi
...
if [ -e agalactiae.ms16.histogram ]; then
  exists1=true
else
  exists1=false
fi

if [ -e agalactiae.ms16.dump ]; then
  exists2=true
else
  exists2=false
fi

if [ -e agalactiae.ms16.ignore.gz ]; then
  exists3=true
else
  exists3=false
fi
if [ $exists1 = true -a $exists2 = true -a $exists3 = true ] ; then
  echo "Output files 'agalactiae.ms16.histogram', 'agalactiae.ms16.dump' and 'agalactiae.ms16.ignore.gz' exist in 'correction/0-mercounts'."
  exit 0
fi
...
if [ ! -e ./agalactiae.ms16/merylIndex ] ; then
  /home/jovyan/software/canu/Linux-amd64/bin/meryl threads=2 memory=2 \
    greater-than 1 \
      output agalactiae.ms16.WORKING \
      union-sum  \
        ./agalactiae.01.meryl \
  && \
  mv -f ./agalactiae.ms16.WORKING ./agalactiae.ms16
  ...
  rm -rf ./agalactiae.01.meryl ./agalactiae.01.meryl.err
fi
...
if [ ! -e ./agalactiae.ms16.histogram ] ; then
  /home/jovyan/software/canu/Linux-amd64/bin/meryl threads=1 memory=1 \
    statistics ./agalactiae.ms16 \
  > ./agalactiae.ms16.histogram
fi
...
if [ ! -e ./agalactiae.ms16.dump ] ; then
  /home/jovyan/software/canu/Linux-amd64/bin/meryl threads=2 memory=2 \
    print ./agalactiae.ms16.##.dump \
      at-least threshold=80 \
      at-least word-frequency=0.0000001 \
        ./agalactiae.ms16

  cat ./agalactiae.ms16.??.dump > ./agalactiae.ms16.dump
  rm -f ./agalactiae.ms16.??.dump
fi
...
if [ ! -e ./agalactiae.ms16.ignore.gz ] ; then
  numKmers=`wc -l < ./agalactiae.ms16.dump`
  totKmers=`/home/jovyan/software/canu/Linux-amd64/bin/meryl statistics ./agalactiae.ms16 | grep present | awk '{ print $2 }'`


  ./meryl-make-ignore.pl $numKmers $totKmers < ./agalactiae.ms16.dump | gzip -1c > ./agalactiae.ms16.ignore.gz
fi
