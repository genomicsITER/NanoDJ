...
if [ -e ./agalactiae.$jobid.meryl/merylIndex ] ; then
...
if [ -e agalactiae.$jobid.meryl.tar ]; then
  exist1=true
else
  exist1=false
fi
...
/home/jovyan/software/canu/Linux-amd64/bin/meryl k=16 threads=2 memory=2 \
  count \
    segment=$jobid/01 ../../agalactiae.seqStore \
    output ./agalactiae.$jobid.meryl.WORKING \
&& \
mv -f ./agalactiae.$jobid.meryl.WORKING ./agalactiae.$jobid.meryl
